...
test('CSV for ISP - Partial', function(assert) {
    ...
    assert.strictEqual(
        this.$('.export-tool-textarea').val(),
        svJoin([
            'Age,Ethnicity,SocialStatus,Religion1to10,Gender,BirthCountry,ReligionFollow,BirthCity,Residence,Language,ReligionYesNo',
            '32,"Special",6,1,1,"The Moon",null,"Big Crater",1,"Tagalog",2'
        ])
    );
});
...
test('CSV for ISP - Partial, Special case 3-3-response form', function(assert) {
    ...
    assert.strictEqual(
        this.$('.export-tool-textarea').val(),
        svJoin([
            'PosNeg,SitSimilarity,BBI16,BBI6,BBI3,BBI1,BBI10,BBI2,BBI8,BBI9,BBI4,BBI5,BBI15,BBI7,BBI13,BBI11,BBI14,BBI12,Risk',
            '6,4,8,3,7,4,7,7,7,3,4,5,4,7,6,4,7,4,0'
        ])
    );
});
...
test('CSV for ISP - Full Example', function(assert) {
    ...
    assert.strictEqual(
        ...
        svJoin([
            ...
            '32,"Special",6,1,1,"The Moon",null,"Big Crater",1,"Tagalog",2,"Daydreaming","Eating something","alone","15:00",6,4,8,3,7,4,7,7,7,3,4,5,4,7,6,4,7,4,0,1,5,1,4,1,1,1,1,3,4,2,5,4,1,4,4,5,5,2,5,5,2,2,1,5,5,5,1,1,2,5,2,1,1,2,5,2,1,4,5,4,1,5,5,5,4,2,5,1,2,5,4,5,5,5,4,3,1,1,5,6,7,7,7,1,1,1,4,4,1,1,1,1,2,2,2,1,3,2,2,3,8,1,2,3,2,3,2,4,3,3,4,2,null,null,5,3,4,3,2,4,3,3,3,4,5,1,2,3,4,1,4,2,3,2,3,4,2,4,1,2,3,2,4,3,3,3,2,2,3,2,4,2,2,4,3,2,2,3,4,3,1,3,6,4,5,1,6,6,3,7,5,3,7,6,4,4,5,3,5,4,3,5,5,4,4,3,5,3,5,4,5,7,6,2,5,5,7,3,6,9,7,7,6,5,6,5,8,8,1,4,6,3,8,7,5,6,8,4,7,5,2,5,5,9,1,5,2,2,5,8,2,4,4,7,6,6,2,7,3,6,4,5,6,3,9,4,3,7,4,6,8,4,3,1,2,1,2,2,2,3,2,1,2,3,2,2,2,2,2,2,2,2,2,2,2,2,2,2,1,2,2,2,2,2,1,2,2,3,1,2,2,3,3,2,2,3,1,2,3,2,1,2,3,1,3,3,2,1,3,2,3,2,1,2,3,1,2,1,1,2,3,1,2,1,3,2,2,1,3,1,1,2,2,3,3,2,1,3,2,2,3,2'
        ])
    );
});
